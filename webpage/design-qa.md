...
- The hero now uses a dark, clearly-labelled concept render that belongs to the black/orange system. It visibly shows the product name, the physical `SEND` control, USB-C connection, and the Online workstation → BLE text transfer → USB keyboard → Isolated computer flow. The supplied enclosure reference still informs the stated 55 × 32 × 17 mm concept size.
...
- **Image quality and asset fidelity:** the visual uses a dedicated high-resolution, dark concept render and is explicitly labelled as a concept render rather than a production photograph.
